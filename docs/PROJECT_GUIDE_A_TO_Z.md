...
- `/ai/status`: Active AI provider mode, Gemini toggle, Gemma4/Ollama toggle, reachability, and fallback order.
...
- `AI_PROVIDER=auto`: try Gemini, then Gemma4/Ollama only when `GEMMA4_ENABLED=true`, then heuristic fallback.
...
The Imports page shows the current provider mode, Gemini readiness, and whether Gemma4 is enabled. Graph 1 draft cards also show `provider_used`, fallback reasons, parse warnings, and schema validation status.
...
- `eval/`: Seeded-case evaluation harness.
