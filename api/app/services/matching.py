...
class MatchingService:
    def recommend(
        ...
        result = self.generate_candidates_for_case(case, teams, volunteers, resources, max_results=max_results)
        return result.recommendations, result.unassigned_reason
    ...
    def generate_candidates_for_case(
        ...
        if case.extracted_json is None:
            return CandidateGenerationResult(
                conflicts=["Incident has not been extracted."],
                unassigned_reason="Case must be extracted before recommendations can be generated.",
            )
        ...
        missing_resources = self._missing_resources(case.extracted_json.required_resources, resources)  # type: ignore[arg-type]
        if missing_resources:
            rejected_conflicts.append(f"Missing available resource stock: {', '.join(missing_resources)}.")
        ...
        for team in teams:  # type: ignore[assignment]
            if team.availability_status == AvailabilityStatus.OFFLINE:
                rejected_conflicts.append(f"{team.team_id} rejected: team offline.")
                continue
            members = [volunteer_lookup[item] for item in team.member_ids if item in volunteer_lookup]
            roster_capacity = max(len(members), len(team.member_ids), 1)
            if not members:
                rejected_conflicts.append(
                    f"{team.team_id} has no active volunteer records; using team-level availability."
                )

            skill_match = self._team_capability_fit(required_skills, team, members)
            if skill_match <= 0:
                rejected_conflicts.append(f"{team.team_id} rejected: capability mismatch.")
                continue
            ...
            capacity_ok = 1.0 if team.active_dispatches < roster_capacity else 0.0
            if capacity_ok == 0.0:
                rejected_conflicts.append(f"{team.team_id} rejected: no available capacity.")
                continue
            workload_balance = max(0.0, 1.0 - (team.active_dispatches / roster_capacity))
        ...
        if not feasible:
            return CandidateGenerationResult(
                conflicts=list(dict.fromkeys(rejected_conflicts)),
                unassigned_reason="No feasible team/resource combination is available right now.",
            )
        primary = [item[0] for item in feasible[:max_results]]
        reserve = self._reserve_recommendations(primary, [item[0] for item in feasible[max_results:]], max_results=2)
        return CandidateGenerationResult(
            recommendations=primary,
            reserve_recommendations=reserve,
            conflicts=list(dict.fromkeys(rejected_conflicts))[:12],
        )

    def plan_for_case_batch(
        self,
        ...
        return {
            case.case_id: self.generate_candidates_for_case(case, teams, volunteers, resources)
            for case in cases
        }
    ...
    def _missing_resources(self, needs: list[ResourceNeed], inventory: list[ResourceInventory]) -> list[str]:
        missing: list[str] = []
        for need in needs:
            if not any(resource.resource_type == need.resource_type and resource.quantity_available > 0 for resource in inventory):
                missing.append(need.resource_type)
        return list(dict.fromkeys(missing))

    def _reserve_recommendations(
        self,
        primary: list[Recommendation],
        candidates: list[Recommendation],
        max_results: int,
    ) -> list[Recommendation]:
        selected_teams = {item.team_id for item in primary if item.team_id}
        reserves: list[Recommendation] = []
        for candidate in candidates:
            if candidate.team_id in selected_teams:
                continue
            reserves.append(candidate)
            if len(reserves) >= max_results:
                break
        return reserves
