...
class Settings(BaseSettings):
    ...
    def resolved_ai_provider(self) -> str:
        ...
        if provider not in {"auto", "gemini", "ollama", "heuristic"}:
            return "auto"
        if provider == "ollama" and not self.gemma4_enabled:
            return "heuristic"
        return provider
    ...
    def provider_fallback_order(self) -> list[str]:
        ...
        if provider == "ollama":
            return ["ollama", "heuristic"] if self.gemma4_enabled else ["heuristic"]
        if provider == "heuristic":
            return ["heuristic"]
        order: list[str] = []
        if self.gemini_enabled:
            order.append("gemini")
        if self.gemma4_enabled:
            order.append("ollama")
