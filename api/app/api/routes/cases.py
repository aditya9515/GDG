...
def create_case(
    ...
    source_hash = duplicate_service.source_hash("INCIDENT", payload.raw_input)
    if not payload.force:
        duplicate = duplicate_service.find_exact_duplicate("INCIDENT", payload.raw_input, actor.active_org_id or "unassigned", repository.list_cases())
    ...
    repository.save_vector_records(
        [
            VectorRecord(
                ...
                org_id=case.org_id or actor.active_org_id or "unassigned",
...
def assign_case(
    ...
    case = repository.get_case(case_id)
    _require_case_org(case, actor)
    saved_recommendations = getattr(repository, "recommendations", {}).get(case_id, []) if hasattr(repository, "recommendations") else []
    recommendation = None
    if payload.recommendation_id:
        recommendation = next((item for item in saved_recommendations if item.recommendation_id == payload.recommendation_id), None)
    if recommendation is None:
        recommendation = saved_recommendations[0] if saved_recommendations else None
    ...
    assignment = AssignmentDecision(
        ...
        reserve_team_ids=payload.reserve_team_ids,
