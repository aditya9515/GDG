...
def create_incident_route(payload: CreateCaseRequest, actor: UserContext = Depends(get_current_user)):
...
def get_incident_route(case_id: str):
    return get_incident(case_id)


@router.post("/{case_id}/extract", response_model=ExtractCaseResponse)
async def extract_incident_route(case_id: str, actor: UserContext = Depends(get_current_user)):
    return await extract_incident(case_id, actor)


@router.post("/{case_id}/score", response_model=ScoreCaseResponse)
def score_incident_route(case_id: str, actor: UserContext = Depends(get_current_user)):
    return score_incident(case_id, actor)


@router.post("/{case_id}/dispatch-options", response_model=RecommendationsResponse)
async def incident_dispatch_options(case_id: str, actor: UserContext = Depends(get_current_user)):
    return await dispatch_options(case_id, actor=actor)


@router.post("/{case_id}/dispatch", response_model=AssignCaseResponse)
def dispatch_incident(case_id: str, payload: AssignCaseRequest, actor: UserContext = Depends(get_current_user)):
    return assign_dispatch(case_id, payload, actor)


@router.post("/{case_id}/location", response_model=CaseDetailResponse)
def incident_location(case_id: str, payload: UpdateLocationRequest, actor: UserContext = Depends(get_current_user)):
    return update_incident_location(case_id, payload, actor)


@router.post("/{case_id}/merge", response_model=MergeCaseResponse)
def merge_incident_route(case_id: str, payload: MergeCaseRequest, actor: UserContext = Depends(get_current_user)):
