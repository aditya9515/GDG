...
const SESSION_EXPIRED_EVENT = 'reliefops:session-expired'

export type OperationalCacheGroup =
  | 'all'
  | 'dashboard'
  | 'incidents'
  | 'cases'
...
const cacheGroupPrefixes: Record<Exclude<OperationalCacheGroup, 'all'>, string[]> = {
  dashboard: ['/dashboard'],
  incidents: ['/incidents'],
...
export function invalidateOperationalCaches(groups: OperationalCacheGroup[] | OperationalCacheGroup = 'all') {
  ...
  normalized.forEach((group) => {
    if (group !== 'all') {
      cacheGroupPrefixes[group].forEach((prefix) => prefixes.add(prefix))
    }
  })
  prefixes.forEach((prefix) => invalidateListCache(prefix))
}
...
export async function getGraphRun(runId: string, session: SessionState | null): Promise<GraphRun> {
  const response = await request<{ run: GraphRun }>(`/agent/runs/${runId}`, {}, session)
  return response.run
}

export async function refreshSessionData(session: SessionState | null): Promise<AuthSessionResponse> {
  return getMe(session)
}

export async function refreshOperationalData(session: SessionState | null, q?: string) {
  const [dashboard, incidents, teams, volunteers, resources, dispatches, ingestionJobs] = await Promise.all([
    getDashboardSummary(session),
    listIncidents(session, q),
    listTeams(session, q),
    listVolunteers(session, q),
    listResources(session, q),
    listDispatches(session, q),
    listIngestionJobs(session, q),
  ])
  return {
    dashboard,
    incidents,
...
export async function createOrganization(name: string, session: SessionState | null) {
  const response = await request<{ organization: Organization; membership: OrgMembership }>(
    '/organizations',
    { method: 'POST', body: JSON.stringify({ name }) },
    session,
  )
  invalidateOperationalCaches(['organizations', 'dashboard'])
  return response
}
...
export async function getOrganizationMembers(orgId: string, session: SessionState | null) {
  return request<{ organization: Organization; members: OrgMembership[] }>(
...
export async function updateOrgMember(
  ...
  const response = await request<{ member: OrgMembership }>(
    `/organizations/${orgId}/members/${membershipId}`,
    { method: 'PATCH', body: JSON.stringify(payload) },
    session,
  )
  invalidateOperationalCaches('organizations')
  return response
}

export async function removeOrgMember(orgId: string, membershipId: string, session: SessionState | null) {
  const response = await request<{ member: OrgMembership; status: string }>(
    `/organizations/${orgId}/members/${membershipId}`,
    { method: 'DELETE' },
    session,
  )
...
export async function runGraph2Batch(payload: Partial<BatchPlanningRequest>, session: SessionState | null) {
  return request<{ run: GraphRun }>(
...
export async function editGraph2(
  runId: string,
  prompt: string,
  draftId: string | null,
  session: SessionState | null,
...
export async function confirmGraph2(runId: string, session: SessionState | null) {
  ...
  invalidateOperationalCaches(['dispatches', 'incidents', 'teams', 'volunteers', 'resources', 'dashboard', 'graphRuns'])
  return response
}

export async function editGraph2BatchCase(
  runId: string,
  caseId: string,
  session: SessionState | null,
  payload: { prompt?: string; field_updates?: Record<string, unknown> },
) {
  return request<{ run: GraphRun }>(
    `/agent/graph2/run/${runId}/edit-case`,
    {
      method: 'POST',
      body: JSON.stringify({
        case_id: caseId,
...
export async function replanGraph2Batch(
  runId: string,
  session: SessionState | null,
  payload: { operator_prompt?: string; reserve_policy?: ReservePolicy | null },
) {
  return request<{ run: GraphRun }>(
    `/agent/graph2/run/${runId}/replan-global`,
    { method: 'POST', body: JSON.stringify(payload) },
    session,
  )
}

export async function confirmGraph2Batch(runId: string, session: SessionState | null) {
  const response = await request<{ run: GraphRun }>(`/agent/graph2/run/${runId}/confirm-batch`, { method: 'POST' }, session)
  invalidateOperationalCaches(['dispatches', 'incidents', 'teams', 'volunteers', 'resources', 'dashboard', 'graphRuns'])
  return response
}

export async function confirmGraph2BatchCase(runId: string, caseId: string, session: SessionState | null) {
  const response = await request<{ run: GraphRun }>(
    `/agent/graph2/run/${runId}/confirm-case`,
...
export async function deleteIncident(caseId: string, session: SessionState | null) {
  ...
  invalidateOperationalCaches(['incidents', 'dashboard'])
  return response
}
...
export async function updateIncidentLocation(
  ...
  const response = await request<CaseDetailResponse>(
...
export async function dispatchIncident(
  ...
  payload: {
    ...
    reserve_team_ids?: string[]
  },
  session: SessionState | null,
) {
  const response = await request(`/incidents/${caseId}/dispatch`, { method: 'POST', body: JSON.stringify(payload) }, session)
  invalidateOperationalCaches(['dispatches', 'incidents', 'teams', 'volunteers', 'resources', 'dashboard'])
  return response
}
...
export async function createTeam(
  ...
  invalidateOperationalCaches(['teams', 'dashboard'])
  return response
}
...
export async function deleteTeam(teamId: string, session: SessionState | null) {
  ...
  invalidateOperationalCaches(['teams', 'volunteers', 'dashboard'])
  return response
}
...
export async function createVolunteer(
  ...
  invalidateOperationalCaches(['volunteers', 'teams', 'dashboard'])
  return response
}
...
export async function createResource(
  ...
  invalidateOperationalCaches(['resources', 'dashboard'])
  return response
}
...
export async function createIngestionJob(
  ...
  if (shouldUseStorage) {
    ...
    invalidateOperationalCaches(['ingestionJobs', 'incidents', 'teams', 'resources', 'dashboard'])
...
export async function deleteIngestionJob(jobId: string, session: SessionState | null) {
  ...
  invalidateOperationalCaches('ingestionJobs')
  return response
}
...
export async function deleteGraphRun(runId: string, session: SessionState | null) {
  ...
  invalidateOperationalCaches(['ingestionJobs', 'graphRuns'])
  return response
}
