          ...
          {(['ALL', 'CRITICAL', 'HIGH', 'NEEDS_REVIEW'] as const).map((item) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-xs font-semibold tracking-[0.18em] ${
                filter === item ? 'bg-foreground text-background' : 'bg-white/5 text-slate-300'
              }`}
      ...
      <section className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        <div className="rounded-[1.5rem] border border-white/8 bg-slate-950/45 p-5">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            >
              {busy ? <InlineLoading label="Running" /> : 'Create + Triage'}
            </button>
            <p className="text-sm text-slate-400">{message ?? 'The triage pipeline creates the case, extracts key facts, scores priority, and prepares dispatch options.'}</p>
          </div>
        </div>
          ...
          <div className="mt-4 grid gap-3">
            {filteredCases.slice(0, 16).map((item) => (
              <Link
                ...
                className="rounded-[1.25rem] border border-white/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.03),rgba(255,255,255,0.01))] p-4 transition hover:border-foreground/25 hover:bg-white/6"
              >
                  ...
                  <div className="space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {item.duplicate_status !== 'NONE' ? (
                        <span className="rounded-full border border-foreground/30 bg-foreground/[0.06] px-2.5 py-1 text-[11px] uppercase tracking-[0.18em] text-foreground">
                          {humanizeToken(item.duplicate_status)}
                        </span>
                      ) : null}
                    </div>
                    <h3 className="font-semibold text-stone-100">{item.case_id}</h3>
                    <p className="line-clamp-2 text-sm leading-6 text-slate-400">{incidentSummary(item)}</p>
                  </div>
